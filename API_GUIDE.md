...
- **Лиды (Чтение)**: Публичное API **не позволяет** просматривать список чужих лидов. Мы можем только создавать новые. Для выгрузки всей базы лидов обычно требуется Manager API (отдельный доступ).
- **Авторизация**: Для Client API используется связка `APP_TOKEN` + номер телефона + SMS-код.
- **База данных**: Сейчас мы храним данные в `gocrm.db`. Это удобно для локального анализа, но для командной работы данные позже переедут в облако (Supabase).
